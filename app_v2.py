...
class digital_twin:
  ...
  def __bubble_chart__(self,data,axs):
    ...
    sns.scatterplot(x=xlabels,y=ylabels,size=score,alpha=0.5,sizes=(50,4000),legend=False, ax=axs)
  ...
  def digital_image(self,id):
    ...
    # combined image
    final_insight, roles = self.__combined_image__(id, career_predicted, mental_risk, physical_risk)

    # flow diagram of a year
    st.subheader("The Digital Sketch")
    text = f'<p style="color:Black; font-size: 18px; font-weight: 500; font-family: Georgia, serif;">Choose a year to get your Digital-Sketch</p>'
    st.markdown(text, unsafe_allow_html=True)
    
    dates = list(self.dates)
    dates.reverse()
    
    year = st.selectbox("Choose a Year",dates)
    indx = self.dates.index(year)
    
    total_experience = f"{int(int(self.data_list[indx].iloc[id]['Years Of Experience Before']) + (int(self.data_list[indx].iloc[id]['Num Months In Company']) / 12))} years {int(self.data_list[indx].iloc[id]['Num Months In Company']) % 12} months"
    
    with Diagram(f"Digital Sketch of {self.data_list[indx].iloc[id]['Email Id'].split('@')[0].title()} for Year: {year}", show=False, filename="data/images/digital_twin", direction='TB'):
      human_image = Custom(f"Employee Id: {id + 1}, Age: {self.data_list[indx].iloc[id]['Age Group']}\nEducation: {self.data_list[indx].iloc[id]['Education'].title()}\nYears of Experience: {total_experience}", f"../images/{self.data_list[indx].iloc[id]['Email Id'].split('@')[0]}.png")
      ...
      if self.dates[indx] == self.dates[-1]:
        with Cluster("Possible Best-Fits"):
          for career in set(career_predicted[indx]):
      ...
      for risk in set(mental_risk[indx]):
        mental_image - Custom("", f"../images/{risk}.png")

      digital_image >> physical_image

      for risk in set(physical_risk[indx]):
  ...
  def overview(self,):
    ...
    kpi2.metric(label = "Avg Attendance", value = np.round(self.data_list[-1]['Percent Present'].mean(),2), delta = f'{change}%')
    ...
    kpi3.metric(label = "Avg Monthly Expense (Rs.)", value = np.round(self.data_list[-1]['Monthly Avg Income'].sum(),2), delta = f'{change}%')
    ...
    kpi4.metric(label = "Avg Overall Rating", value = np.round(self.data_list[-1]['Overall Rating'].mean(),2), delta = f'{change}%')
